...
class Recorder(SpanRecorder):
    ...
    def __init__(self,
        ...
        tracer_tags.update({
            ...
            'zipkin_ot.tracer_version': tracer_version.OPENZIPKIN_OT_PYTHON_TRACER_VERSION,
        ...
        runtime_attrs = [thrift.KeyValue(k, util._coerce_str(v)) for (k, v) in tracer_tags.iteritems()]
        ...
        if self._periodic_flush_seconds <= 0:
            warnings.warn(
                'Runtime(periodic_flush_seconds={0}) means we will never flush to zipkin_ot unless explicitly requested.'.format(
        ...
        if span.tags:
            for key in span.tags:
                if key[:len(constants.JOIN_ID_TAG_PREFIX)] == constants.JOIN_ID_TAG_PREFIX:
                    span_record.join_ids.append(thrift.TraceJoinId(key, util._coerce_str(span.tags[key])))
                else:
                    span_record.attributes.append(thrift.KeyValue(key, util._coerce_str(span.tags[key])))
    ...
    def _flush_worker(self, connection):
        ...
        try:
            ...
            for i, encoded_span in enumerate(report_request.span_records):
            ...
            self._finest("Received response from collector: %s", (r.status_code,))
    ...
    def _construct_report_request(self):
        ...
        report = None
        with self._mutex:
            report = thrift.ReportRequest(self._runtime, self._span_records,
                                          None)
            self._span_records = []
